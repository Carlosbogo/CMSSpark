...
##H How to test: Just provide a test directory as output directory. If you want to make a full test, don't forget to set PARQUET_LOCATION different.
##H
TZ=UTC
START_TIME=$(date +%s)
myname=$(basename "$0")
...
spark_submit_args=(
    ...
    --driver-memory=16g --executor-memory=32g
...
# run spark function
function run_spark() {
    util_spark_submit_with_otel_logs "${spark_submit_args[@]}" "$script_dir/dbs_hdfs_eos.py" $@
}
