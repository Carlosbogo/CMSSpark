...
set -e
##H cron4crab_unique_users.sh
##H    Creates CRAB unique users plots
...
##H
##H Example: cron4crab_unique_users.sh --keytab ./keytab --output <DIR> --p1 32000 --p2 32001 --host $MY_NODE_NAME --wdir $WDIR
##H Arguments:
##H   - keytab             : Kerberos auth file: secrets/keytab
##H   - output             : Output directory. If not given, $HOME/output_crab_uu will be used. I.e /eos/user/c/cmsmonit/www/crab_uu
...
run_spark --generate_plots --by "month" --output_folder "$OUTPUT_DIR" --start_date "$START_DATE" --end_date "$END_DATE" 2>&1
run_spark --generate_plots --by "weekofyear" --output_folder "$OUTPUT_DIR" --start_date "$START_DATE" --end_date "$END_DATE" 2>&1
