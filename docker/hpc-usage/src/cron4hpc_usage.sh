...
ls -l /opt/spark-apps/hpc-usage/
ls -l /etc/secrets/
# ls -l /eos/user/c/cmsmonit/www/hpc_usage || echo "Output dir not found"
##H cron4hpc_usage.sh
...
ls -l /eos/user/c/cmsmonit/www/hpc_usage || echo "Output dir not found"
# ----------------------------------------------------------------------------------------------------------------- RUN
HTML_TEMPLATE="$script_dir"/html_template.html
...
spark_submit_args=(
    ...
    # --conf spark.appMasterEnv.PYTHONPATH=$PYTHONPATH
    # --conf spark.appMasterEnv.LD_LIBRARY_PATH=$LD_LIBRARY_PATH
